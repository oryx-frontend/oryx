...
import { hydrate, I18nMixin, Size } from '@oryx-frontend/utilities';
  ...
  implements QuantityInputAttributes, QuantityInputMethods
{
  ...
  protected override render(): TemplateResult {
    return html`
      <button
        @click=${this.decrease}
        aria-label=${this.i18n('input.quantity.decrease')}
        tabindex="-1"
      ...
      >
        <input
          ${ref(this.inputRef)}
          aria-label=${this.i18n('input.quantity')}
          type="number"
          .value=${this.getValue()}
          min=${this.min}
      ...
      <button
        @click=${this.increase}
        aria-label=${this.i18n('input.quantity.increase')}
        tabindex="-1"
